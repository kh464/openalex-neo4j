...
> 注意：本文保留为大规模导入的历史分析文档。当前代码已经取消“limit > 5000 时自动限制 `expand_depth=1` 并跳过 `referenced_works`”的硬限制，同时 `search_works()` 也已显式取消 PyAlex 默认的 `10000` 条结果上限。
...
| 方案 | 效果 | 实现复杂度 |
|---|---|---|
| **A: 历史方案 — 大导入自动限制 expand_depth=1** | 跳过引用展开，节省 80%+ 调用 | 低 |
| **B: 批量 DOI filter** | 合并多个 DOI 为一次查询 | 低（仅适用于 WoS 路径） |
| **C: 跳过关系展开（仅入库）** | 零额外调用 | 低 |
| **D: 延迟展开** | 先入库后再后台展开 | 中 |
| **E: 并行请求** | 提效 3-5x | 中（需处理限频） |

**历史方案推荐组合**：A + C。当时的实现会在 `limit > 5000` 时自动限制 `expand_depth=1` 并跳过引用展开，只抓取直接作者/来源/主题。当前代码已移除这一硬限制。
...
|---|---|---|
| API 搜索 | 50s | 50s |
| 关系展开（depth=1，历史方案：跳过引用） | ~5min | 5min |
| JSONL 写入 | ~10s | 10s |
...
**在历史方案里，不展开引用关系时，10 万篇的瓶颈不在代码，在 OpenAlex API 限频（~5 分钟）和可选的 embedding 生成（~30 分钟）**。
...
| 优先级 | 优化项 | 预期效果 | 工作量 |
|---|---|---|---|
| P0 | 历史方案：limit > 5000 时自动限制 expand_depth=1 | 防止引用爆炸 | 0.5d |
| P0 | 历史方案：大导入跳过引用展开（referenced_works） | 节省 80%+ API 调用 | 0.5d |
...
对于十万级导入，**核心瓶颈不在代码逻辑，而在于 OpenAlex API 限频和大规模关系展开。** 当前架构仍主要依赖以下能力：

1. 按类型分批读写 JSONL 降低内存
2. 给用户清晰的进度反馈
3. 由 `expand_depth` 明确控制关系展开范围，而不是按抓取规模自动跳过引用展开
